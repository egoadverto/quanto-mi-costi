...
function Campo({
  ...
  return (
    <label className="block space-y-1">
      ...
      {children}
      <span className="text-xs text-slate-500">{aiuto}</span>
    </label>
  );
}
...
function App() {
  ...
  const helper = 'w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm';
    ...
    return (
      <main className="mx-auto max-w-md p-4">
        ...
        <form onSubmit={signIn} className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <Campo label="Email" aiuto="Inserisci l'email dell'account. Formato: nome@dominio.it (esempio: mario.rossi@email.it).">
            <input className={helper} type="email" placeholder="nome@dominio.it" value={email} onChange={(e) => setEmail(e.target.value)} required />
          </Campo>
          <Campo label="Password" aiuto="Inserisci la password del tuo account. Esempio: una password che usi già per l'app.">
    ...
    <main className="mx-auto max-w-6xl space-y-5 p-4">
      <header className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-slate-600">
          <li>I consumi sono stimati sui rifornimenti registrati.</li>
          <li>Più dati inserisci, più le medie diventano utili.</li>
        </ul>
      </header>
      ...
      <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          <p className="mb-2 text-sm font-semibold">Efficienza media per veicolo</p>
      ...
      <section className="space-y-4">
        <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form onSubmit={saveVeicolo} className="space-y-3">
            <Campo label="Nome veicolo*" aiuto="Inserisci un nome facile da riconoscere. Formato: testo libero (esempio: Auto famiglia).">
              <input className={helper} placeholder="Es. Auto famiglia" required value={veicoloForm.nome} onChange={(e) => setVeicoloForm({ ...veicoloForm, nome: e.target.value })} />
            </Campo>
            <Campo label="Marca" aiuto="Inserisci la marca del veicolo. Formato: testo (esempio: Fiat).">
              <input className={helper} placeholder="Es. Fiat" value={veicoloForm.marca} onChange={(e) => setVeicoloForm({ ...veicoloForm, marca: e.target.value })} />
            </Campo>
            <Campo label="Modello" aiuto="Inserisci il modello del veicolo. Formato: testo (esempio: Panda).">
              <input className={helper} placeholder="Es. Panda" value={veicoloForm.modello} onChange={(e) => setVeicoloForm({ ...veicoloForm, modello: e.target.value })} />
            </Campo>
            <Campo label="Odometro iniziale" aiuto="Inserisci i km iniziali. Formato: numero (esempio: 52340).">
              <input className={helper} type="number" placeholder="Es. 52340" value={veicoloForm.odometro_iniziale} onChange={(e) => setVeicoloForm({ ...veicoloForm, odometro_iniziale: e.target.value })} />
            </Campo>
            <Campo label="Tipo veicolo" aiuto="Seleziona la tipologia. Formato: auto o moto (esempio: auto).">
            ...
            <Campo label="Tipo energia" aiuto="Seleziona l'alimentazione. Formato: elettrico, benzina o diesel (esempio: benzina).">
            ...
            <Campo label="Unità predefinita" aiuto="Seleziona l'unità usata più spesso. Formato: L o kWh (esempio: L).">
            ...
            <Campo label="Note" aiuto="Aggiungi dettagli opzionali. Formato: testo libero (esempio: gomme nuove a marzo).">
              <textarea className={helper} placeholder="Es. Uso prevalente in città" value={veicoloForm.note} onChange={(e) => setVeicoloForm({ ...veicoloForm, note: e.target.value })} />
            </Campo>
            <div className="flex gap-2">
              <button className="flex-1 rounded-md bg-slate-900 p-2.5 font-medium text-white" type="submit">{editingVeicoloId ? 'Aggiorna' : 'Aggiungi veicolo'}</button>
              {editingVeicoloId && <button className="rounded-md border border-slate-300 bg-slate-50 px-3" type="button" onClick={() => { setEditingVeicoloId(null); setVeicoloForm(veicoloInit); }}>Annulla</button>}
            </div>
          </form>
          <ul className="mt-4 space-y-2 text-sm">
            {veicoli.map((v) => (
              <li key={v.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form onSubmit={saveRifornimento} className="space-y-3">
            <Campo label="Veicolo" aiuto="Seleziona il veicolo a cui associare il rifornimento. Esempio: Auto famiglia.">
            ...
            <Campo label="Data" aiuto="Inserisci la data dell'operazione. Formato: gg/mm/aaaa (esempio: 26/04/2026).">
            ...
            <Campo label="Odometro" aiuto="Inserisci i km al momento del rifornimento. Formato: numero (esempio: 54020).">
              <input className={helper} required type="number" placeholder="Es. 54020" value={rifornimentoForm.odometro} onChange={(e) => setRifornimentoForm({ ...rifornimentoForm, odometro: e.target.value })} />
            </Campo>
            <Campo label="Quantità" aiuto="Inserisci litri o kWh acquistati. Formato: numero > 0 (esempio: 32.5).">
              <input className={helper} required type="number" step="0.01" placeholder="Es. 32.5" value={rifornimentoForm.quantita} onChange={(e) => setRifornimentoForm({ ...rifornimentoForm, quantita: e.target.value })} />
            </Campo>
            <Campo label="Unità" aiuto="Seleziona l'unità della quantità. Formato: L o kWh (esempio: L).">
            ...
            <Campo label="Prezzo unitario" aiuto="Inserisci prezzo per litro/kWh. Formato: numero ≥ 0 (esempio: 1.7890).">
              <input className={helper} required type="number" step="0.0001" placeholder="Es. 1.7890" value={rifornimentoForm.prezzo_unitario} onChange={(e) => setRifornimentoForm({ ...rifornimentoForm, prezzo_unitario: e.target.value })} />
            </Campo>
            <Campo label="Costo totale" aiuto="Inserisci il totale pagato. Formato: numero ≥ 0 (esempio: 58.14).">
              <input className={helper} required type="number" step="0.01" placeholder="Es. 58.14" value={rifornimentoForm.costo_totale} onChange={(e) => setRifornimentoForm({ ...rifornimentoForm, costo_totale: e.target.value })} />
            </Campo>
            <Campo label="Fornitore" aiuto="Inserisci stazione o colonnina. Formato: testo (esempio: Q8 Via Roma).">
              <input className={helper} placeholder="Es. Q8 Via Roma" value={rifornimentoForm.fornitore} onChange={(e) => setRifornimentoForm({ ...rifornimentoForm, fornitore: e.target.value })} />
            </Campo>
            <Campo label="Note" aiuto="Aggiungi dettagli utili. Formato: testo libero (esempio: traffico intenso).">
              <textarea className={helper} placeholder="Es. Viaggio autostradale" value={rifornimentoForm.note} onChange={(e) => setRifornimentoForm({ ...rifornimentoForm, note: e.target.value })} />
            </Campo>
            <div className="flex gap-2">
              <button className="flex-1 rounded-md bg-slate-900 p-2.5 font-medium text-white" type="submit">{editingRifornimentoId ? 'Aggiorna' : 'Aggiungi rifornimento'}</button>
              {editingRifornimentoId && <button className="rounded-md border border-slate-300 bg-slate-50 px-3" type="button" onClick={() => { setEditingRifornimentoId(null); setRifornimentoForm(rifornimentoInit); }}>Annulla</button>}
            </div>
          </form>
          <ul className="mt-4 space-y-2 text-sm">
            {rifornimenti.map((r) => (
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form onSubmit={saveSpesa} className="space-y-3">
            <Campo label="Veicolo" aiuto="Seleziona il veicolo collegato alla spesa. Esempio: Auto famiglia.">
            ...
            <Campo label="Data" aiuto="Inserisci la data della spesa. Formato: gg/mm/aaaa (esempio: 26/04/2026).">
              <input className={helper} required type="date" value={spesaForm.data} onChange={(e) => setSpesaForm({ ...spesaForm, data: e.target.value })} />
            </Campo>
            <Campo label="Categoria" aiuto="Seleziona il tipo di spesa. Esempio: manutenzione, bollo, assicurazione.">
            ...
            <Campo label="Descrizione" aiuto="Inserisci una descrizione breve. Formato: testo (esempio: cambio olio).">
              <input className={helper} placeholder="Es. Cambio olio e filtro" value={spesaForm.descrizione} onChange={(e) => setSpesaForm({ ...spesaForm, descrizione: e.target.value })} />
            </Campo>
            <Campo label="Importo" aiuto="Inserisci il costo totale della spesa. Formato: numero ≥ 0 (esempio: 129.90).">
              <input className={helper} required type="number" step="0.01" placeholder="Es. 129.90" value={spesaForm.importo} onChange={(e) => setSpesaForm({ ...spesaForm, importo: e.target.value })} />
            </Campo>
            <Campo label="Odometro (opzionale)" aiuto="Inserisci i km al momento della spesa. Formato: numero (esempio: 54100).">
              <input className={helper} type="number" placeholder="Es. 54100" value={spesaForm.odometro} onChange={(e) => setSpesaForm({ ...spesaForm, odometro: e.target.value })} />
            </Campo>
            <Campo label="Note" aiuto="Aggiungi dettagli utili. Formato: testo libero (esempio: sconto applicato).">
              <textarea className={helper} placeholder="Es. Officina di fiducia" value={spesaForm.note} onChange={(e) => setSpesaForm({ ...spesaForm, note: e.target.value })} />
            </Campo>
            <div className="flex gap-2">
              <button className="flex-1 rounded-md bg-slate-900 p-2.5 font-medium text-white" type="submit">{editingSpesaId ? 'Aggiorna' : 'Aggiungi spesa'}</button>
              {editingSpesaId && <button className="rounded-md border border-slate-300 bg-slate-50 px-3" type="button" onClick={() => { setEditingSpesaId(null); setSpesaForm(spesaInit); }}>Annulla</button>}
            </div>
          </form>
          <ul className="mt-4 space-y-2 text-sm">
            {spese.map((s) => (
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="mb-3 text-lg font-semibold">Storico dati</h2>
        <p className="text-sm text-slate-600">
          Lo storico mostra le registrazioni inserite nelle sezioni Veicoli, Rifornimenti / Ricariche e Spese.
          Usa i pulsanti “Modifica” o “Elimina” in ogni card per aggiornare rapidamente i dati.
        </p>
      </section>
    </main>
  );
